					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<select class="form-control auto-select" data-selected="{{ $loanproduct->visibility }}" name="visibility" required>
						...
						@foreach($transactionFees as $transactionFee)
							...
							<div class="col-md-2">
								<div class="control-label">
									<input type="checkbox" id="{{ 'pro'.$transactionFee->id }}" name="transaction_fee[]" value="{{ $transactionFee->id }}" {{ ( $loanproduct_transactionFee->contains($transactionFee->id) )? "checked" : "" }}>
									<label class="control-label" for="{{ 'pro'.$transactionFee->id }}">{{ $transactionFee->name }}</label>
								</div>
							</div>
						@endforeach
						...
						<div class="col-md-12">
							<div class="form-group">
								<button type="submit" class="btn btn-primary"><i class="fa-solid fa-circle-check"></i> {{ _lang('Update Changes') }}</button>
							</div>
						</div>
					</div>
				</form>
