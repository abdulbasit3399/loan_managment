	...
	<div class="col-lg-12">
		<div class="card no-export">
		    <div class="card-header">
				...
				<a class="btn btn-primary btn-sm float-right ajax-modal" data-title="{{ _lang('Add New Transaction Fee') }}" href="{{ route('transaction_fees.create') }}"><i class="icofont-plus-circle"></i> {{ _lang('Add New') }}</a>
			</div>
			<div class="card-body">
								...
								<span class="dropdown">
								  ...
								  <form action="{{ action('TransactionFeeController@destroy', $transactionFee['id']) }}" method="post">
									...
									<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
										<a href="{{ action('TransactionFeeController@edit', $transactionFee['id']) }}" data-title="{{ _lang('Update Loan Purpose Details') }}" class="dropdown-item dropdown-edit ajax-modal"><i class="icofont-ui-edit"></i> {{ _lang('Edit') }}</a>
										{{-- <a href="{{ action('TransactionFeeController@show', $transactionFee['id']) }}" data-title="{{ _lang('Loan Purpose Details') }}" class="dropdown-item dropdown-view ajax-modal"><i class="icofont-eye-alt"></i> {{ _lang('View') }}</a> --}}
