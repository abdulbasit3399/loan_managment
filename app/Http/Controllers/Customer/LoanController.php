...
class LoanController extends Controller {
    ...
    public function apply_loan(Request $request) {
      ...
      if ($request->isMethod('get')) {
        ...
        $loan_products = LoanProduct::where('status',1)->whereIn('visibility',$visibility)->get();
        return view('backend.customer_portal.loan.apply_loan',compact('loan_products'));
      } else if ($request->isMethod('post')) {
        ...
        $prev_loan = Loan::latest('created_at')->first();
        if($prev_loan)
          $loan_id = $prev_loan->loan_id + 1;
        else{
          $setting = Setting::where('name','loan_id')->first();
          $loan_id = $setting->value + 1;
        }
        $attachment = "";
    ...
    public function loan_payment(Request $request, $loan_id) {
      if (request()->isMethod('get')) {
        ...
        $name = '';
        if($request->hasfile('recipet'))
        {

          $this->validate($request,[
            'recipet' => 'nullable|image'
          ]);

          $file = $request->file('recipet');
          $name = time().$file->getClientOriginalName();
          $file->move(public_path().'/uploads/recipet/',$name);
        }
