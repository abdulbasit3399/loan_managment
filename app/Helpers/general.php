...
if (!function_exists('kyc_count')) {
    function kyc_count($html = true) {
        $count = \App\Models\User::where("user_type", 'customer')
        ...
        if ($count > 0) {
            return '<span class="sidebar-notification-count">' . $count . '</span>';
        }
    }
}
...
if (!function_exists('get_logo_light')) {
    function get_logo_light() {
        $logo = get_option("logo");
        if ($logo == "") {
            return asset("backend/images/company-logo.png");
        }
        return asset("uploads/media/logo-light.png");
    }
}
...
if (!function_exists('request_count')) {
    function request_count($request, $html = false, $class = "sidebar-notification-count") {
        ...
        } else if ($request == 'internal_transfer_requests') {
            $notification_count = \App\Models\Transaction::whereRaw("(type = 'Transfer' OR type = 'Exchange')")
